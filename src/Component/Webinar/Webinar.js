...
import "./webinar.css";
function Webinar() {
    const [webinar, setWeb] = useState([]);
    const [categories, setcat] = useState([]);
    useEffect(() => {
        const fetch = async () => {
            const web = await axios.get(`http://127.0.0.1:8080/webinar/get`);
            setWeb(web.data)
        }
        const fetch2 = async () => {
    ...
    const useItems = webinar.map((item) => {
        return (
            <div className="col-lg-4 col-md-6 mt-5">
                <div className="card d-flex flex-column align-items-center">
                    <div className="product-name"><Link to={`/webinar/slug=${item.slug}`} params={{ slug: item.slug }}>{item.title}</Link></div>
