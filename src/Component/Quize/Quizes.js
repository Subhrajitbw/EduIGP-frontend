...
function Quizes() {
    ...
    useEffect(() => {
        const fetch = async () => {
            const quizes = await axios.get(`http://127.0.0.1:8080/quiz/get`);
    ...
    const useItems = quizes.map((item) => {
        return (
            <div className="col-lg-4 col-md-6 mt-5">
                <div className="card d-flex flex-column align-items-center h-100">
                    <div className="product-name mb-4"><Link to={`/quiz/slug=${item.slug}`} params={{slug: item.slug}}>{item.title}</Link></div>
                    <div className="card-img">
                        <img class="thumbnail" src={`data:image/png;charset=utf-8;base64,${item.image.data}`} alt="" />
                        <div className="caption">
                            <button className="btn btn-diff">By: {item.organizer}</button>
                        </div>
                    </div>
